...
def detect_faces(net:nn.Module, img:np.ndarray, minscale:int=3, ovr_threshhold:float=0.3,
                 score_threshhold:float=0.5, device:str='cuda') -> List[Tuple]:
    ...
    """
    bboxlist = detect(net, img, minscale, device)
...
def detect(net:nn.Module, img:np.ndarray, minscale:int=3, device:str='cuda') -> torch.Tensor:
